...
import yamlesque.{ArrayVisitor, Ctx, ObjectVisitor, Parser, Visitor}
...
class UjsonDelegate[A](d: upickle.core.Visitor[_, A]) extends Visitor[A] {
  ...
  def visitString(ctx: Ctx, text: CharSequence): A = d.visitString(text, ctx.pos.index)
  def visitBool(ctx: Ctx, value: Boolean): A =
    if (value) d.visitTrue(ctx.pos.index) else d.visitFalse(ctx.pos.index)
  def visitNumber(ctx: Ctx, text: CharSequence): A =
    if (Parser.isIntegerLiteral(text)) {
      // keep integers exact, e.g. for Long fields
      d.visitFloat64StringParts(text.toString.stripPrefix("+"), -1, -1, ctx.pos.index)
    } else {
      d.visitFloat64(Parser.parseNumber(text), ctx.pos.index)
    }
}
